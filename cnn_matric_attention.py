...
class RNAConvNet(nn.Module):
    def __init__(self, embedding_dim=2560, num_heads=8):
        ...
        # 定义自注意力层
        self.self_attention = nn.MultiheadAttention(embed_dim=256, num_heads=num_heads)
        ...
        self.fc = nn.Linear(256, 2)  # 假设num_classes是输出类别的数量
    ...
    def forward(self, x):
        ...
        # 自注意力层
        # 需要调整x的形状以匹配自注意力的输入要求 [sequence_length, batch_size, embed_dim]
        x = x.permute(2, 0, 1)  # [N, C, L] -> [L, N, C]
        x, _ = self.self_attention(x, x, x)  # 自注意力层
...
train_data_folder = './data/embedding/mouse/train'  # 假设您的数据在这个文件夹中
train_batch_size =16 # 根据您的内存限制调整批次大小
...
x_var,y_var=load_all_tensors_to_matrix('./data/embedding/mouse/val/')
# x_var = x_var.astype(float)
# X_vartrain=torch.tensor(x_var)
# y_vartrain = torch.tensor(y_var)
var_dataset = TensorDataset(x_var, y_var)
var_loader = DataLoader(dataset=var_dataset, batch_size=16, shuffle=True)
...
#model_path = './data/embedding/nnj/model/model_cnn_attion8.pth'
model_path = './data/embedding/mouse/model/model_cnn_attion8.pth'
# 保存模型的状态字典
